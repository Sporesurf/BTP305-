...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		if (file)
		{
			...
			{
				if (tempStr[0] != '#' && counter < 4) // first 4 books
				{
					library[counter] += Book(tempStr);
					counter++;
				}
			}
			/*
	...
	auto priceConversion = [usdToCadRate, gbpToCadRate](Book& book) -> void
	{
	...
	if (argc > 2) {
		...
		if (file)
		{
			while (getline(file, tempStr))
			{
				if (tempStr[0] != '#' && counter < 5)
				{
					movies[counter] = Movie(tempStr);
					counter++;
				}
			}
		}
